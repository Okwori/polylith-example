...
(describe "GET /indicators/:id"
  (describe "when document is found"
    (it "returns HTTP 200"
      (with-redefs [store/find-document (constantly mock-document)
    ...
    (it "passes the path id to find-document"
      (let [captured-id (atom nil)]
        (with-redefs [store/find-document (fn [_db id _policy]
  ...
  (describe "when document is not found"
    (it "returns HTTP 404"
      (with-redefs [store/find-document (constantly nil)
